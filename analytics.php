...
try {
    ...
    if (!empty($league_id)) {
        ...
        // Sort participants alphabetically so order is stable across date range changes
        sort($trackingParticipants);
...
// Generate distinct colors — use the FULL sorted participant list as the stable
// color palette so colors never shift when the date range changes
const allParticipantsSorted = <?= json_encode(array_values($participantInfo)) ?>;
const generateColor = (i, t) => `hsl(${(i * 360 / t) % 360}, 70%, 55%)`;
const participantColorMap = {};
allParticipantsSorted.forEach((name, i) => {
    participantColorMap[name] = generateColor(i, allParticipantsSorted.length);
});

// Build chart datasets
const datasets = trackingParticipants.map((p) => ({
    label: p,
    data: trackingChartData[p],
    borderColor: participantColorMap[p],
    backgroundColor: participantColorMap[p] + '20',
    fill: false,
    tension: 0,
    borderWidth: 2,
    pointRadius: 3,
...
<?php endif; ?>
    ...
    <?php $currentPage = 'analytics'; include '/data/www/default/nba-wins-platform/components/pill_nav.php'; ?>
</body>
</html>