<?php
// nba-wins-platform/auth/login_v2.php
// V2 Login - Supports users with no leagues (redirects to league hub)
// Runs side-by-side with existing login.php
...
if ($auth->isAuthenticated()) {
    ...
    if (empty($userLeagues) && !$auth->isGuest()) {
...
if ($_POST) {
    ...
    if (empty($username) || empty($password)) {
        ...
        // Custom login that handles no-league users
        try {
            $stmt = $pdo->prepare("
            ...
            if ($user && password_verify($password, $user['password_hash'])) {
                ...
                // Get default league (may be NULL for new users)
                $defaultLeague = null;
                ...
                // Store session — use NULL (not false/empty string) when no league
                $stmt = $pdo->prepare("
                ...
                $stmt->execute([
                    $sessionId,
                    $user['id'],
                    $defaultLeague, // NULL if no leagues — not false or ''
...
<body>
    <div class="login-container">
        ...
        <a href="login_v2.php?guest=1" class="guest-btn">
            <i class="fas fa-eye"></i> Browse as Guest
        </a>

        <div class="register-link">
            Don't have an account? <a href="register_v2.php">Create one here</a>
        </div>
    </div>
</body>
</html>